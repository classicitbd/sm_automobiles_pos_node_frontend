...
const CheckOutPayment = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/supplier_payment/check_or_cash_out_payment?supplier_payment_method=check&page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
    ],
    queryFn: async () => {
      try {
        const res = await fetch(
          `${BASE_URL}/supplier_payment/check_or_cash_out_payment?supplier_payment_method=check&page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
          {
            credentials: "include",
          }
        );
