          ...
          <div className="mt-2">
            ...
            <div className="flex justify-between p-3 bg-gray-50 shadow-md mt-4 flex-wrap gap-2">
              <div className="font-bold">
                <p className="sm:text-[18px] text-bgray-700">User Name : {saleTargetDetails?.data?.user_id?.user_name}</p>
                <p className="sm:text-[18px] text-bgray-700">
                  User Phone : {saleTargetDetails?.data?.user_id?.user_phone}
                </p>
                <p className="sm:text-[18px] text-bgray-700">
                  User Address : {saleTargetDetails?.data?.user_id?.user_address}
                </p>
                <p className="sm:text-[18px] text-bgray-700">
                  User Status :
              ...
              <div className="font-bold">
                ...
                <p className="sm:text-[18px] text-bgray-700">
                  Total Target : <span className="text-blue-600 sm:text-[18px]">{saleTargetDetails?.data?.sale_target}{" "}{settingData?.unit_name}</span>
                </p>
                <p className="sm:text-[18px] text-bgray-700">
                  Get Amount : <span className="text-green-600 sm:text-[18px]">{saleTargetDetails?.data?.sale_target_amount}</span>
                </p>
                <p className="sm:text-[18px] text-bgray-700">
                  Fill Up :

                  {
                    saleTargetDetails?.data?.sale_target_filup >= saleTargetDetails?.data?.sale_target ? <span className="text-green-600 sm:text-[18px]"> {saleTargetDetails?.data?.sale_target_filup}{" "}
                      {settingData?.unit_name}</span> : <span className="text-red-600 sm:text-[18px]"> {saleTargetDetails?.data?.sale_target_filup}{" "}
                      {settingData?.unit_name}</span>
                  }
                </p>
                <p className="sm:text-[18px] text-bgray-700">
                  Target Status : {saleTargetDetails?.data?.sale_target_success == true ? <span className="text-green-600 sm:text-[18px]">Success</span>
                    : <span className="text-blue-600 sm:text-[18px]">Pending</span>}
                </p>
              </div>
            </div>
          </div>
                      ...
                      (sale_target, i) => (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap py-1.5 font-medium text-green-600">
                            ...
                            {
                              sale_target?.received_amount === sale_target?.grand_total_amount ? (
                                <span className="text-green-600">{sale_target?.received_amount}</span>
                              ) : sale_target?.received_amount > sale_target?.grand_total_amount ? (
                                <span className="text-blue-600">{sale_target?.received_amount}</span>
                              ) : (
                                <span className="text-yellow-600">{sale_target?.received_amount}</span>
                              )
                            }
                          </td>
