...
const PurchaseDocumentModalShow = ({
      ...
      <div>
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-[750px] p-6 max-h-[100vh] overflow-y-auto scrollbar-thin'>
            ...
            <div>
              {getDocumentData?.purchase_voucher?.endsWith('.pdf') ||
              getDocumentData?.purchase_voucher?.endsWith('.PDF') ? (
                <iframe
                  src={getDocumentData?.purchase_voucher}
                  width='100%'
                  height='500'
                  title='PDF Preview'
                />
                ...
                <img
                  src={getDocumentData?.purchase_voucher}
                  alt={getDocumentData?.purchase_title}
                />
              )}
            </div>
          </div>
...
export default PurchaseDocumentModalShow
